...
const submitStatusEl = document.getElementById('submit-status');
    ...
    function generateReportPdfBase64() {
        ...
        if (!reportElement) {
            console.error("PDF Generation Error: reportElement #completion-area not found.");
            return Promise.reject(new Error("Report element #completion-area not found for PDF generation."));
        }

        console.log("Attempting html2canvas on #completion-area. OffsetWidth:", reportElement.offsetWidth, "OffsetHeight:", reportElement.offsetHeight, "ScrollWidth:", reportElement.scrollWidth, "ScrollHeight:", reportElement.scrollHeight);

        return html2canvas(reportElement, {
            scale: 1.5, // 提高清晰度
            useCORS: true,
            logging: true, // Enable logging for html2canvas
            windowWidth: reportElement.scrollWidth, // Use scrollWidth to capture full content
            windowHeight: reportElement.scrollHeight // Use scrollHeight to capture full content
        }).then(canvas => {
            console.log("html2canvas success. Canvas dimensions: width =", canvas.width, "height =", canvas.height);

            if (canvas.width === 0 || canvas.height === 0) {
                console.error("PDF Generation Error: Canvas has zero dimension.");
                return Promise.reject(new Error("Canvas generated by html2canvas has zero dimension."));
            }
            ...
            const a4WidthMM = 210;
            const a4HeightMM = 297; // Standard A4 height

            // Let jsPDF handle the page orientation and initial format based on A4
            const pdf = new jsPDF({
            ...
            const imgData = canvas.toDataURL('image/jpeg', 0.85);
            console.log("imgData length for PDF:", imgData.length);
            ...
            // Ensure imgWidth and imgHeight are valid numbers
            if (typeof imgWidth !== 'number' || typeof imgHeight !== 'number' || imgWidth <= 0 || imgHeight <= 0) {
                console.error("PDF Generation Error: Invalid image properties from getImageProperties.", imgProps);
                return Promise.reject(new Error("Invalid image properties for PDF generation."));
            }
            const aspectRatio = imgWidth / imgHeight;


            // Calculate the width and height for the image on the PDF page
            // to fit within A4, maintaining aspect ratio
            // Page margins (e.g., 10mm each side, 10mm top/bottom)
            const pageMargin = 10;
            const availableWidth = a4WidthMM - (2 * pageMargin);
            const availableHeight = a4HeightMM - (2 * pageMargin);

            let pdfImgWidth = availableWidth;
            let pdfImgHeight = pdfImgWidth / aspectRatio;

            // If calculated height is too much for available height, then scale by height instead
            ...
            if (pdfImgWidth <= 0 || pdfImgHeight <= 0) {
                console.error("PDF Generation Error: Calculated image dimensions for PDF are zero or negative.", {pdfImgWidth, pdfImgHeight});
                return Promise.reject(new Error("Calculated image dimensions for PDF are invalid."));
            }

            // Center the image
            const xOffset = (a4WidthMM - pdfImgWidth) / 2;
            const yOffset = pageMargin; // Top margin

            console.log("Adding image to PDF with dimensions (mm): width=", pdfImgWidth, "height=", pdfImgHeight, "at x=", xOffset, "y=", yOffset);
            pdf.addImage(imgData, 'JPEG', xOffset, yOffset, pdfImgWidth, pdfImgHeight);
            console.log("Image added to PDF.");

            const dataUri = pdf.output('datauristring');
            console.log("PDF output as data URI successful.");
